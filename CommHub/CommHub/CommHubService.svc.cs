...
namespace CommHub
{
    public class CommHubService : IService
    {
        ...
        #region Comm_Read
        public wsResponse<Comm_Read_Resp> Comm_Read(wsRequest<InstanceID> req)
        {
            var funcName = "Comm_Read";
            var errCode = 0;
            var errorText = "";

            var results = new wsResponse<Comm_Read_Resp>();
            var resp = new Comm_Read_Resp();
            var dc = new DataHubDataContext();
            long ownerHubID = 0;
            ...
            try
            {
                ...
                foreach (Comm_ReadResult itm in dc.Comm_Read(req.Params.id, ownerHubID))
                {
                    resp.id = itm.id;
                    resp.groupID = itm.groupID ?? 0;
                    resp.link = itm.link;
                    resp.name = itm.name;
                    resp.photoLink = itm.photoLink;
                    resp.photoLinkBig = itm.photoLinkBig;
                    resp.subjectCommID_name = itm.subjectCommID_name;
                    resp.adminCommID_linkFB = itm.adminCommID_linkFB;
                    resp.adminCommID_Name = itm.adminCommID_Name;
                    resp.areaCommID_code = itm.areaCommID_code;
                    resp.countMembers = itm.countMemebers ?? 0;
                    resp.countMen = itm.countMen ?? 0;
                    resp.countMenPercent = itm.countMenPercent ?? 0;
                    resp.countWoman = itm.countWoman ?? 0;
                    resp.countWomanPercent = itm.countWomanPercent ?? 0;
                };
            ...
            {
                errCode = 101;
                string param = string.Format("ownerHubID: {0}, commID: {1}", ownerHubID, req.Params.id);
                Tools.ErrorLog_Save(req, param, funcName, e.Message);
        ...
        #region ProjectHub_Del
        public wsResponse ProjectHub_Del(wsRequest<InstanceID> req)
        {
            var funcName = "ProjectHub_Del";
            var errCode = 0;
            var errorText = "";
            ...
            try
            {
                ...
                dc.ProjectHub_Del(id, ownerHubID);
        ...
        #region ProjectHub_ReadDict
        public wsResponse<ProjectHub_ReadDict_Resp> ProjectHub_ReadDict(wsRequest req)
        {
            var funcName = "ProjectHub_ReadDict";
            var errCode = 0;
            var errorText = "";

            var results = new wsResponse<ProjectHub_ReadDict_Resp>();
            var resp = new ProjectHub_ReadDict_Resp();
            var dc = new DataHubDataContext();
            long ownerHubID = 0;
            ...
            try
            {
                ...
                foreach (ProjectHub_ReadDictResult itm in dc.ProjectHub_ReadDict(ownerHubID))
                {
                    resp.Add(new wsProject()
                    {
                        id = itm.id,
                        name = itm.name
                    });
        ...
        #region ProjectHub_Save
        public wsResponse ProjectHub_Save(wsRequest<ProjectHub_Save_Req> req)
        {
            var funcName = "ProjectHub_Save";
            var errCode = 0;
            var errorText = "";
            ...
            {
                errCode = 101;
                string param = string.Format("id: {0}, ownerHubID: {1}, name: {2}", id, ownerHubID, name);
                Tools.ErrorLog_Save(req, param, funcName, e.Message);
        ...
        {
            var funcName = "StaCommDaily_Report";
            var errCode = 0;
            var errorText = "";
            ...
            try
            {
                ...
                foreach (StaCommDaily_ReportResult itm in dc.StaCommDaily_Report(ownerHubID, req.Params.isPast))
                {
        ...
        public wsResponse<StaCommVK_Report_Resp> StaCommVKDaily_Report(wsRequest<StaCommDaily_Report_Req> req)
        {
            ...
            try
            {
                ...
                foreach (StaCommVKDaily_ReportResult itm in dc.StaCommVKDaily_Report(ownerHubID, req.Params.isPast))
                {
                    resp.Add(new wsSta()
                    {
                        projectHub_id = itm.projectHub_id ?? 0,
                        projectHub_name = itm.projectHub_name,
                        comm_id = itm.comm_id,
                        comm_name = itm.comm_name,
                        comm_photoLink = itm.comm_photoLink,
                        ...
                        increase = itm.increase,
                        increaseNew = itm.increaseNew,
                        increaseOld = itm.increaseOld,
                        increaseDifPercent = itm.increaseDifPercent ?? 0,
